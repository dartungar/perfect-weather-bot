from climate_dicts import mean_max_temp_range_dict, sunshine_hours_range_dict, precipitation_monthly_range_dict, mean_humidity_range_dict
import db
...
def select_iwmos_by_weather(month='Jan', 
    ...
    sel = select([c_normals, iwmos]).where(and_(c_normals.c.iwmo == iwmos.c.iwmo_id, 
                                               ...
                                               # c_normals.c.mean_temp_mean_value.between(mean_temp_mean_range[0], mean_temp_mean_range[1]),
                                                ...
                                                c_normals.c.sunshine_mean_number_of_hours.between(sunshine_hours_range[0], sunshine_hours_range[1])
                                                ))
...
def get_iwmos_by_climate_dict(month='Jan', mean_max_temp='none', humidity='none', precipitation_monthly='none', sunshine='none'):
...
if __name__ == '__main__':
    ...
    iwmos = get_iwmos_by_climate_dict(month='Jan', mean_max_temp='freezing', sunshine='sunny')
